...
use smithay::{
    desktop::{PopupUngrabStrategy, layer_map_for_output, space::SpaceElement}, input::{Seat, pointer::MotionEvent}, output::Output, reexports::wayland_server::{Resource, protocol::wl_surface::WlSurface}, utils::{IsAlive, Point, SERIAL_COUNTER, Serial}, wayland::{
        ...
        for output in self.outputs().cloned().collect::<Vec<_>>().into_iter() {
            ...
            let has_foreground_fullscreen = self.seats.iter().any(|seat| {
                if let Some(fs) = workspace.get_fullscreen(seat) {
                    ...
                    let focus_stack_is_valid_fullscreen = last_focused.is_some_and(|target| {
                        match target {
                    ...
                    let seat_active_here = seat.focused_output().is_some_and(|active_output| {
