...
impl CosmicSurface {
    ...
    pub fn surface_tree_offset(
        ...
        let found = AtomicBool::new(false);
        let found_offset = Mutex::new(Point::<i32, Logical>::default());
        ...
        with_surface_tree_downward(
            ...
            |s, states, parent_offset| {
                ...
                if s != root {
                    offset += states.cached_state.get::<SubsurfaceCachedState>().current().location;
                }
                TraversalAction::DoChildren(offset)
            },
            |s, _, offset| {
                if s == surface {
                    *found_offset.lock().unwrap() = *offset;
        ...
        if found.load(Ordering::SeqCst) {
            return Some(*found_offset.lock().unwrap());
        }
