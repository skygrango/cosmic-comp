...
void main() {
    ...
    vec4 sum = sample_color(uv + vec2(-half_pixel.x * 2.0, 0.0) * offset);
    sum += sample_color(uv + vec2(-half_pixel.x, half_pixel.y) * offset) * 2.0;
    sum += sample_color(uv + vec2(0.0, half_pixel.y * 2.0) * offset);
    sum += sample_color(uv + vec2(half_pixel.x, half_pixel.y) * offset) * 2.0;
    sum += sample_color(uv + vec2(half_pixel.x * 2.0, 0.0) * offset);
    sum += sample_color(uv + vec2(half_pixel.x, -half_pixel.y) * offset) * 2.0;
    sum += sample_color(uv + vec2(0.0, -half_pixel.y * 2.0) * offset);
    sum += sample_color(uv + vec2(-half_pixel.x, -half_pixel.y) * offset) * 2.0;
    ...
    if (sum.a <= 0.0001) {
        ...
        if (params.is_hdr != 0) {
            vec3 lin = sum.rgb / 12.0;
            color = vec4(linear_to_pq(lin), sum.a / 12.0);
        } else {
            color = sum / sum.a;
        }
    }
