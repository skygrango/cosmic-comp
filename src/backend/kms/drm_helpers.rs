...
mod hdr_tests {
    ...
    #[test]
    fn crtc_color_pipeline_types_available() {
        use smithay::backend::drm::{CrtcColorState, DrmColorCtm, DrmColorLut};

        let default_state = CrtcColorState::default();
        assert!(default_state.gamma_lut.is_none());
        assert!(default_state.ctm.is_none());
        assert!(default_state.degamma_lut.is_none());

        let lut = DrmColorLut::create_pq_lut(1024, 203.0);
        assert_eq!(lut.len(), 1024);

        let ctm = DrmColorCtm::rec709_to_bt2020();
        let state = CrtcColorState {
            gamma_lut: Some(lut),
            ctm: Some(ctm),
            degamma_lut: None,
        };
    ...
    #[test]
    fn hardware_offload_selection_and_config() {
        use smithay::backend::renderer::gles::HdrOutputConfig;

        let default_cfg = HdrOutputConfig::default();
        assert!(!default_cfg.hardware_offload);

        let custom = HdrOutputConfig::new(300.0, 2.2, 0.0, 1000.0, true);
        assert!(custom.hardware_offload);
        assert_eq!(custom.reference_white, 300.0);
    }
}
