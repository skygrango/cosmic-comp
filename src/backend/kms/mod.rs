...
use render::{gles::GbmGlowBackend, vulkan::GbmVulkanBackend};
use smithay::{
    backend::{
        ...
        renderer::{ImportDma, glow::GlowRenderer, multigpu::GpuManager},
...
pub enum KmsGpuApi {
    ...
    Vulkan {
        instance: Instance,
        api: GpuManager<GbmVulkanBackend<DrmDeviceFd>>,
    },
}

impl KmsGpuApi {
    pub fn is_vulkan(&self) -> bool {
        matches!(self, KmsGpuApi::Vulkan { .. })
    }

    pub fn vulkan_instance(&self) -> Option<&Instance> {
        match self {
            KmsGpuApi::Vulkan { instance, .. } => Some(instance),
            KmsGpuApi::Glow(_) => None,
        }
    }

    pub fn dmabuf_formats(&mut self, node: &DrmNode) -> Option<FormatSet> {
        match self {
            KmsGpuApi::Glow(api) => Some(api.single_renderer(node).ok()?.dmabuf_formats()),
            KmsGpuApi::Vulkan { api, .. } => Some(api.single_renderer(node).ok()?.dmabuf_formats()),
        }
    }
    ...
    pub fn reevaluate(&mut self, primary_node: Option<&DrmNode>) {
        if let Some(primary_node) = primary_node {
            match self {
                KmsGpuApi::Glow(api) => {
                    let _ = api.single_renderer(primary_node);
                }
                KmsGpuApi::Vulkan { api, .. } => {
                    let _ = api.single_renderer(primary_node);
                }
    ...
    pub fn allow_overlay_scanout(
        ...
        match self {
            KmsGpuApi::Glow(api) => {
                let mut renderer = api.single_renderer(&device.inner.render_node)?;
                device.allow_overlay_scanout(flag, &mut renderer, clock, shell)
            }
            KmsGpuApi::Vulkan { api, .. } => {
                let mut renderer = api.single_renderer(&device.inner.render_node)?;
                device.allow_overlay_scanout(flag, &mut renderer, clock, shell)
...
pub fn init_backend(
    ...
    let api = if use_vulkan {
        ...
        KmsGpuApi::Vulkan { instance, api }
    } else {
        info!("Initializing GL KMS backend");
        KmsGpuApi::Glow(
...
impl KmsState {
    ...
    pub fn dmabuf_imported(
        ...
        let new_client = if let Some(client) = client {
            ...
            let updated = match &mut self.api {
                KmsGpuApi::Glow(api) => device
                    .inner
                    .update_egl(self.primary_node.read().unwrap().as_ref(), api.as_mut())?,
                KmsGpuApi::Vulkan { api, .. } => device
                    .inner
                    .update_vulkan(self.primary_node.read().unwrap().as_ref(), api.as_mut())?,
            };
            updated && new
        } else {
            false
    ...
    pub fn schedule_render(&mut self, output: &Output) {
        self.schedule_backend(output, false);
    }

    pub fn schedule_render_fullscreen(&mut self, output: &Output) {
        self.schedule_backend(output, true);
    }

    pub fn schedule_backend(&mut self, output: &Output, is_fullscreen: bool) {
        for surface in self
            .drm_devices
            .values()
        ...
        {
            surface.schedule_backend(is_fullscreen);
        }
    }
    ...
    fn clear_used_devices(&mut self) -> Result<()> {
        ...
        self.api.reevaluate(primary_node.as_ref());
    ...
    pub fn refresh_used_devices(&mut self) -> Result<()> {
        ...
        for device in self.drm_devices.values_mut() {
            let res = match &mut self.api {
                KmsGpuApi::Glow(api) => device
                    .inner
                    .update_egl(primary_node.as_ref(), api.as_mut())?,
                KmsGpuApi::Vulkan { api, .. } => device
                    .inner
        ...
        for device in self.drm_devices.values_mut() {
            for surface in device.inner.surfaces.values_mut() {
                ...
                if (white - previous).abs() > f32::EPSILON {
                    surface.prepare_hdr_rendering(true, white, surface.hdr_hardware_offload);
            ...
            for (crtc, surface) in device.inner.surfaces.iter_mut() {
                ...
                if !test_only {
                    if !surface.is_active() {
                            ...
                            match (
                                        ...
                                        && colorspaces.contains(&Colorspace::Bt2020Rgb) =>
                                {
                                    ...
                                    let state = ConnectorColorState {
                                        ...
                                        max_bpc,
                                        reference_white: Some(hdr_reference_white),
                                    };
                                    match compositor_ref.use_color_state(state) {
                                        Ok(()) => {
                                            ...
                                            info!(
                                                ...
                                                hardware_offload = compositor_ref.hdr_hardware_offload(),
                                                ?max_bpc,
                        ...
                        let crtc_hardware_offload = compositor_ref.hdr_hardware_offload();
                        ...
                        surface.prepare_hdr_rendering(
                            hdr_active,
                            hdr_reference_white,
                            crtc_hardware_offload,
                        );
                        ...
                        if hdr_requested != hdr_active {
                            ...
                            if hdr_requested {
                                ...
                                match (
                                        ...
                                        ) && colorspaces.contains(&Colorspace::Bt2020Rgb) =>
                                    {
                                        ...
                                        match compositor_ref.use_color_state(ConnectorColorState {
                                            ...
                                            max_bpc,
                                            reference_white: Some(white),
                                        }) {
                                            Ok(()) => {
                                                let crtc_hardware_offload =
                                                    compositor_ref.hdr_hardware_offload();
                                                surface.prepare_hdr_rendering(
                                                    true,
                                                    white,
                                                    crtc_hardware_offload,
                                                ...
                                                info!(
                                                    output = %surface.output.name(),
                                                    reference_white = white,
                                                    hardware_offload = crtc_hardware_offload,
                                                    "enabled HDR live"
                                                );
                                            }
                                            Err(err) => warn!(
                                ...
                                surface.prepare_hdr_rendering(false, hdr_reference_white, false);
                                surface
                                    .output
                                    .user_data()
                                    .get::<drm_helpers::HdrOutputState>()
                            ...
                            && (hdr_reference_white - previous_reference_white).abs() > f32::EPSILON
                        {
                            let mut compositor_ref =
                                drm.compositors().get(crtc).unwrap().lock().unwrap();
                            let mut color_state = compositor_ref.pending_color_state();
                            color_state.reference_white = Some(hdr_reference_white);
                            let _ = compositor_ref.use_color_state(color_state);
                            let crtc_hardware_offload = compositor_ref.hdr_hardware_offload();
                            surface.prepare_hdr_rendering(
                                true,
                                hdr_reference_white,
            ...
            if !device.inner.surfaces.is_empty() {
                ...
                match &mut self.api {
                    KmsGpuApi::Glow(api) => {
                        ...
                        device
                            .allow_primary_scanout_any(single_display, &mut renderer, clock, &shell)
