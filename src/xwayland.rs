...
use smithay::{
    ...
    utils::{
        Buffer as BufferCoords, IsAlive, Logical, Point, Rectangle, SERIAL_COUNTER, Serial, Size, Transform
    },
    wayland::{
        selection::{
            SelectionTarget,
    ...
    pub fn launch_xwayland(&mut self, render_node: Option<DrmNode>) {
        ...
        match self
            ...
            .insert_source(xwayland, move |event, _, data| match event {
                ...
                XWaylandEvent::Error => {
                    data.common.xwayland_reset_eavesdropping(SERIAL_COUNTER.next_serial());
                    ...
                    shell.pending_windows.retain(|pending| pending.surface.alive());
                    shell.pending_activations.retain(|key, _| !matches!(key, crate::shell::ActivationKey::X11(_)));
...
impl XwmHandler for State {
    ...
    fn fullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        match shell.fullscreen_request(&window, output.clone(), &self.common.event_loop_handle) {
            ...
            None => {
                if shell.is_surface_mapped(&window) {
                    let target = KeyboardFocusTarget::Fullscreen(window.clone().into());
                    std::mem::drop(shell);
                    Shell::set_focus(self, Some(&target), &seat, None, true);
                } else if let Some(pending) = shell
                    .pending_windows
