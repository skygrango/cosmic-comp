        ...
        match event {
            ...
            InputEvent::PointerMotion { event, .. } => {
                ...
                if let Some(seat) = shell.seats.for_device(&event.device()).cloned() {
                    ...
                    let (output, output_geometry, new_under) = {
                        let shell = self.common.shell.read();
                        let output = shell
                            .outputs()
                        ...
                        let new_under = if pointer_confined
                            && let Some((surface, surface_loc)) = &under
                        {
                            let is_legal = |pos: Point<f64, Global>, shell: &Shell| {
                                // surface_under may be not good here.
                                let is_under = State::surface_under(pos, &output, shell)
                                    .map(|(target, pos)| (target, pos.as_logical()));
                                if let Some((t, _)) = &is_under {
                                    if t.wl_surface() != surface.wl_surface() {
                                        return (false, None);
                                    }
                                }
                        ...
                        (output, output_geometry, new_under)
                    };
